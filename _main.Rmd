...
2) Once the data documentation is all there, you can move the raw file to the Box. 

3) Clean up the code you used to create the "clean" version of the data. Make sure to:
    * include comments describing the methods of cleaning
    * it directly reads the data from the newly saved location
    * it directly saves the data to the `Box/clean` folder, create a relevant subfolder if you're going to save several files from the same source data, or save in an existing subfolder if it feels correct. 
    
4) Name the file informatively and put the code in the [`general_cleaning`](https://github.com/Duke-EJ-Lab/general_cleaning) GitHub repo. Commit and push your code. 

5) Run the code! If you've checked that the paths match the Box location, it should just pop up 
...
million records from AIRS.

**Where**: United States. Eviction lab's own bulk requests were from all
50 states. LexisNexis included records from all states except North and
South Dakota, AIRS provides eviction records from 6 states, Arizona,
California, New Jersey, Nevada, Ohio and Oregon.
...
<!--chapter:end:06-stumbling-blocks.Rmd-->
